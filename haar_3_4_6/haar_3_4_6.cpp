...
int main(int argc, const char** argv) {
	...
	CommandLineParser parser(argc, argv,
		...
		//"{model_cascade|E:/University/10sem/nirs/haar_3_4_6/preparing data/haar_results_v6/cascade.xml|Path to face cascade.}"
		"{model_cascade|E:/University/10sem/nirs/haar_3_4_6/preparing data/haar_results_v2/cascade.xml|Path to cascade No 1.}"
...
void detect_and_display(Mat frame, int cascadeNum) {
	...
	case 1:
		imshow("Cascade 1 detection: 200p 600n 42-48 ", frame);
		if (objects.size() != 0)
			cascadeCounter_1++;
		break;
	case 2:
		imshow("Cascade 1 detection: 1500p 2955n 22-24", frame);
		if (objects.size() != 0)
			cascadeCounter_2++;
		break;
	default:
